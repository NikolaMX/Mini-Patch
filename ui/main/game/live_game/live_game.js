...
$(document).ready(function () {
    ...
    function updateUnitDetails(options){
        $.get("/"+options.id).then(function(result){
            if(result.tools !== undefined){
                _.forEach(result.tools, function(tool){
                    $.get("/"+tool.spec_id).then(function(toolResult){
                        if(toolResult.ammo_source !== undefined){
                            model.unitSpecs[options.id].ammo_source = toolResult.ammo_source
                        }
                        if(toolResult.max_range !== undefined){
                            if(model.unitSpecs[options.id].max_range == undefined){
                                model.unitSpecs[options.id].max_range = toolResult.max_range
                            }
                            else{
                                if(model.unitSpecs[options.id].max_range < toolResult.max_range){
                                    model.unitSpecs[options.id].max_range = toolResult.max_range
                                }
                            }
                        }
    ...
    function UnitDetailModel(options) {
        ...
        self.dps = ko.observable(options.dps);
        self.max_range = ko.observable(options.max_range)
        ...
        self.energy_weapon = ko.observable(options.ammo_capacity ? {
            ammo_source: options.ammo_source || "energy",
    ...
    function LiveGameViewModel() {
        ...
        self.allowCustomFormations = ko.observable(true);
        ...
        self.noHumanPlayers = ko.computed(function () {
            var humanArmies = _.reject(self.players(), 'ai');

            return humanArmies == 0;
        });
        ...
        self.showSettings.subscribe(function(showSettings)
        {
            refreshPanel('settings', showSettings);

            if (showSettings && self.singleHumanPlayer())
                self.pauseSim();

            if (!showSettings)
            {
                self.refreshSettings();
                api.panels.build_bar.message('refresh_settings');
            }
        });
        ...
        self.showPlayerVisionFlags = ko.computed(function () {
            return self.devMode() || self.reviewMode() || self.cheatAllowChangeVision() || self.sandbox();
        });

        self.playerControlFlags = ko.observableArray([]);
        self.showPlayerControlFlags = ko.computed(function () {
            return self.devMode() || self.cheatAllowChangeControl() || self.sandbox();
        });
        ...
        self.timeBarState = ko.computed(function () {
            ...
            var require = self.noHumanPlayers() || self.singleHumanPlayer() || self.gameOptions.sandbox();
        ...
        self.buildItemFromList = function (index, row, column)
        {
        ...
        self.focusedHolodeck = ko.observable();

        /// Camera API
        self.cameraMode = ko.observable('planet');

        self.updateCameraMode = function() {
            var holodeck = api.Holodeck.focused;

            var cameraMode = holodeck.cameraMode();

            var existingCameraMode = model.cameraMode();

            if (existingCameraMode == cameraMode)
                return;
        ...
        self.trackCommander = function(holodeck, armyId) {
            ...
            if (holodeck) {
                ...
                if (holodeck) {
                    if (self.pips.length > 0 && holodeck === self.pips[0])
                        self.showPips(true);
            ...
            if (armyId)
                api.select.armyCommanders(armyId);
            else
        ...
        self.showSocial = ko.observable(false).extend({ session: 'show_social' });

        self.showUberBar = self.showSocial;
        ...
        self.updateUberBarVisibility = self.updateSocialVisibility;
            ...
            if (mdevent.button === 0) {
                ...
                input.capture(holodeck.div, function (event) {
                    ...
                    if (dragCommand === "" && event.type === 'mousemove' && eventTime >= dragTime)
                    {
                        ...
                        input.release();
                        if (dragCommand === 'move' || dragCommand === 'unload')
                        {
        ...
        // allow for selection in game over mode when reviewing
        // TODO: cleanup this mess

        holodeckModeMouseDown['game_over'] = function (holodeck, mdevent) {
            scaleMouseEvent(mdevent);
        ...
        self.playerListState = ko.computed(function() {
            return {
                spectator: self.isSpectator(),
                playerWasAlwaysSpectating: self.playerWasAlwaysSpectating(),
                army: self.armyId(),
        ...
        self.devModeState = ko.computed(function() {
            if (!self.showDevControls())
                return {};
            return {
                cheatVision: self.devMode() || self.cheatAllowChangeVision() || self.sandbox(),
                cheatControl: self.devMode() || self.cheatAllowChangeControl() || self.sandbox(),
                players: self.sendablePlayers(),
                vision: self.playerVisionFlags(),
        ...
        self.serverRate = ko.observable(1.0);
        self.simSpeedMultiplier = ko.observable(1);

        self.increaseSimSpeed = function() {
            var sim_speed_multiplier = self.simSpeedMultiplier() + 0.5;
            if (sim_speed_multiplier > 5)
                return;
            model.send_message('set_sim_speed_multiplier', {sim_speed_multiplier: sim_speed_multiplier});
        }

        self.decreaseSimSpeed = function() {
            var sim_speed_multiplier = self.simSpeedMultiplier() - 0.5;
            if (sim_speed_multiplier < 0.5)
                return;
            model.send_message('set_sim_speed_multiplier', {sim_speed_multiplier: sim_speed_multiplier});
        }
        ...
        self.ammoBuildHover =
        {
            // deprecated as buildable projectiles are now automatically added to unit specs with projectile: true
        };
...
// update the timestamp in reconnect to game info every minute
        ...
        self.togglePOV = function(forcePrimary) {

            var focusedHolodeck = api.Holodeck.focused;
        ...
        self.togglePrimaryPOV = function()
        {
            self.togglePOV(true);
        }

        self.crossRefUnitSpecs = function(units)
        {
            for (var id in units) {
                var unit = units[id];
                unit.id = id;
                ...
                var projectile_specs = unit.projectile_specs;

                if (projectile_specs)
                {
                    for (var p = 0; p < projectile_specs.length; ++p)
                    {
        ...
        self.playerCommanderIds = {}

        self.planetComMessage = []

        self.checkPlayerPlanet = function(){
        ...
        self.sendPlayerComData = function(armyId, armyColor, commanders){
            self.playerCommanderIds[armyId] = [];
            var planets = self.planetListState().planets;
        ...
        self.finishSendingComData = function(armyId, armyColor){
        ...
        self.createCustomSpecialWeaponAlert = function(type,planet){
            if(type == "smashPlanet"){
                if (!model.squelchNotifications()) {
                model.doCustomAlert({
    ...
    handlers.unit_specs = function (payload)
    {
        ...
        model.crossRefUnitSpecs(model.unitSpecs);
        ...
        _.delay(processAsUnitData, 3000, model.unitSpecs)
        ...
        _.forIn(model.unitSpecs, function(unit, id)
        {
            ...
            {
                if(_.some(unit[element] || [], function(target)
                {
                    return target.buildGroup !== 'misc';
                }))
                {
                    unit.canBuild = true;
                }
    ...
    handlers.control_state = function (payload) {
        ...
        if (payload.sim_speed_multiplier)
            model.simSpeedMultiplier(payload.sim_speed_multiplier);
    ...
    handlers['track_commander'] = function(payload) {
        if (!payload)
            payload = {};
        model.trackCommander(payload.holodeck, payload.armyId);
    }
...
api.debug.log('live_game mount_mod_file_data ' + JSON.stringify(payload));
    ...
    handlers.event_message = function (payload) {
        switch (payload.type) {
            ...
            case 'commander_spawn':
                ...
                target.zoom = api.camera.getFocus(model.holodeck.id).zoomLevel() == 'surface' ? 'current' : 'air';
    ...
    handlers.holodeck_focus = function(holodeckId) {
        model.updateCameraMode();
    }
