...
//annihilation mode support
var lastLivingUnit = {}//used to improve search efficiency
var annihilationModeUnitsLoaded = false//so it sets the array one time
var annihilationModeUnitTypes = ["Commander"]//set by lobby settings
...
function executeToLoad(winners, losers) {
    ...
    var control = client_state.control;

    control.valid_time_range.max = sim.time;

    control.finish_timestamp = Date.now();

    control.duration = control.valid_time_range.max - control.valid_time_range.min;
    control.elapsed = (control.finish_timestamp - control.start_timestamp) / 1000;
...
function updateGameOverState(endGame) {
    ...
    var gameOver = true;
    var losers = [];
    var nonSharedAlliance = false;

    // dynamic alliance check
    ...
    _.forEach(armies, function (army) {
        if(!game_options.dynamic_alliance_victory && gameOver){
            _.forEach(armies, function (target) {
                
                if(diplomaticStates[army.id][target.id] !== undefined && army.id !== target.id){
                    if (isAlly(army, target) && diplomaticStates[army.id][target.id].mutable){
                        nonSharedAlliance = true;
                        updateDiplomaticState(army, target.id, 'hostile');
                    }
                }
            });
    ...
    if(nonSharedAlliance == true){gameOver = false}
...
function verifyArmyHasValidUnit(army) {
    if(lastLivingUnit[army.index] == undefined){lastLivingUnit[army.index] = 0}
    var found = false;
    for(var unitIndex = lastLivingUnit[army.index]; unitIndex< sim.units.length;unitIndex++){
        var unit = sim.units[unitIndex]
        for(var unitTypeIndex = 0; unitTypeIndex< annihilationModeUnitTypes.length;unitTypeIndex++){
            if (unit.isUnitType(annihilationModeUnitTypes[unitTypeIndex])) {
                if (unit.army.index === army.index && !unit.dead) {
                    found = true;
                    lastLivingUnit[army.index] = unitIndex
...
function tickDefeatState(check_for_resurrection) {
    ...
    if(!annihilationModeUnitsLoaded && game_options.annihilation_mode){
        annihilationModeUnitTypes = []
        if(game_options.annihilation_mode_support_commanders){annihilationModeUnitTypes.push("Support_Commander")}
        if(game_options.annihilation_mode_factories){annihilationModeUnitTypes.push("Factory")}
        if(game_options.annihilation_mode_fabricators){annihilationModeUnitTypes.push("Fabber")}
        if(annihilationModeUnitTypes.length <1){annihilationModeUnitTypes = ["Commander"]}//if no options are selected it will just be the players commander
        annihilationModeUnitsLoaded = true
    }
    ...
    _.forEach(armies, function (army) {
        ...
        if (game_options.sudden_death_mode){
            alive = ! _.some(army.commanders, function (commander) { return commander.dead; });
        }
        else if(game_options.annihilation_mode){
            var commanderAlive = _.some(army.commanders, function (commander) { return !commander.dead; });
            var validUnitAlive = verifyArmyHasValidUnit(army)
            if(commanderAlive || validUnitAlive){
                alive = true
            }
            else{
                alive = false
...
function playerMsg_setSimSpeedMultiplier(msg) {
...
// for now sim speed can only be set for single player local server live games
    ...
    var sim_speed_multiplier = msg.payload.sim_speed_multiplier;

    if (sim_speed_multiplier < 0.5 || sim_speed_multiplier > 5)
        return response.fail("Sim speed multiplier must be between 0.5 and 5.0");

    sim.speed = sim_speed_multiplier;

    client_state.control.sim_speed_multiplier = sim_speed_multiplier;
...
exports.enter = function (config) {
    ...
    if (!loaded_from_replay) {
        ...
        commandersDone.promise.then(function () {
            ...
            sim.enableSave();
            /* enable save will init the min valid time. we could also just look at sim.time, but this way is more consistent */
            client_state.control.valid_time_range = sim.getValidTimeRange();

            client_state.control.start_timestamp = Date.now();
